...
export default function EntranceAnimation({ onReplay=0 }:{onReplay?:number}){
  ...
  useEffect(() => {
    ...
    const seen = sessionStorage.getItem('protocol:intro') === '1'
    if((seen && onReplay === 0) || reduced) return
    setVisible(true)
    const timer = setTimeout(() => { setVisible(false); sessionStorage.setItem('protocol:intro','1') }, 4300)
  ...
  return <div className="intro" role="dialog" aria-label="Doomsday Protocol initialization">
    ...
    <button className="skip" onClick={()=>{sessionStorage.setItem('protocol:intro','1');setVisible(false)}}>Skip intro</button>
  </div>
}
